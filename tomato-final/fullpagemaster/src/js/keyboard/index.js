...
function isInsideInput() {
  ...
  return (
    utils.matches(activeElement, "textarea") ||
...
function onkeydown(e) {
  ...
  var activeElement = doc.activeElement;
  var isMediaFocused =
    utils.matches(activeElement, "video") ||
    utils.matches(activeElement, "audio");
  var isScrolled = {
...
function onTab(e) {
  ...
  const isLastFocusableInSection =
...
function afterSectionLoads(v) {
  if (
    !utils.closest(g_elToFocus, SLIDE_SEL) ||
    utils.closest(g_elToFocus, SLIDE_ACTIVE_SEL)
  ) {
    focusItem();
  }
}
...
function getFocusables(el) {
  return [].slice
    .call(utils.$(focusableElementsString, el))
    .filter(function (item) {
      return (
        utils.getAttr(item, "tabindex") !== "-1" &&
...
function cancelDirectionKeyEvents(e) {
  if (
    ...
    !utils.closest(e.target, OVERFLOW_SEL)
  ) {
    e.preventDefault();
  }
}
