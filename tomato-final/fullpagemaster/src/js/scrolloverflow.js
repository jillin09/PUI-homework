...
export const scrollOverflowHandler = {
  ...
  makeScrollable: function () {
    ...
    getState().panels.forEach(function (el) {
      ...
      if (
        utils.hasClass(el.item, AUTO_HEIGHT_RESPONSIVE) &&
        isResponsiveMode()
      ) {
        return;
      } else {
        let item = getSlideOrSection(el.item);
        const shouldBeScrollable = scrollOverflowHandler.shouldBeScrollable(
          el.item
        );
  ...
  hasScroll: function (panelItem) {
    return (
      utils.hasClass(panelItem, OVERFLOW) ||
      utils.$(OVERFLOW_SEL, panelItem)[0] != null
    );
  },
