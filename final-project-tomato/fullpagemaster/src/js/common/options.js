...
export function setOptionsFromDOM() {
  ...
  if (!getOptions().anchors.length) {
    ...
    if (
      anchors.length &&
      anchors.length === utils.$(getOptions().sectionSelector, container).length
    ) {
      g_initialAnchorsInDom = true;
      anchors.forEach(function (item) {
        getOptions().anchors.push(
          utils.getAttr(item, "data-anchor").toString()
        );
      });
    }
  }
    ...
    if (tooltips.length) {
      tooltips.forEach(function (item) {
        getOptions().navigationTooltips.push(
          utils.getAttr(item, "data-tooltip").toString()
        );
      });
    }
  }
