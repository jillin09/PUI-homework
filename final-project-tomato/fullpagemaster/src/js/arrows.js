...
function onClickOrTouch(params) {
  ...
  if (
    utils.matches(target, SLIDES_ARROW_SEL) ||
    utils.closest(target, SLIDES_ARROW_SEL)
  ) {
...
export function createSlideArrows(section) {
  ...
  var arrows = [
    utils.createElementFromHTML(getOptions().controlArrowsHTML[0]),
    utils.createElementFromHTML(getOptions().controlArrowsHTML[1]),
  ];
  ...
  if (getOptions().controlArrowColor !== "#fff") {
    utils.css(utils.$(SLIDES_ARROW_NEXT_SEL, sectionElem), {
      "border-color":
        "transparent transparent transparent " + getOptions().controlArrowColor,
    });
    utils.css(utils.$(SLIDES_ARROW_PREV_SEL, sectionElem), {
      "border-color":
        "transparent " +
...
export function toggleControlArrows(v) {
  if (!getOptions().loopHorizontal && getOptions().controlArrows) {
    ...
    utils.toggle(
      utils.$(SLIDES_ARROW_NEXT_SEL, v.section),
      utils.next(v.destiny) != null
    );
  }
}
