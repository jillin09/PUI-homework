...
const pointers = {
  ...
  touchstart:
    "ontouchstart" in window ? "touchstart" : MSPointer ? MSPointer.down : null,
};
...
function touchMoveHandler(e) {
  var activeSection =
    utils.closest(e.target, SECTION_SEL) || getState().activeSection.item;
  ...
  if (isReallyTouch(e)) {
    ...
    var isHorizontalMovementEnough =
      ...
      (utils.getWindowWidth() / 100) * getOptions().touchSensitivity;
    var isHorizontalPredominantMove =
      utils.$(SLIDES_WRAPPER_SEL, activeSection).length &&
